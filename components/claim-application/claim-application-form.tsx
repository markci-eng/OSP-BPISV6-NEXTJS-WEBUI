...
import { Body, Box, Checkbox } from "st-peter-ui";
...
const ClaimApplicationForm = () => {
  ...
  const [claimTypeCheckBoxList, setClaimTypeCheckBoxList] = React.useState<
    CheckBoxData[]
  >([
    ...
    {
      label: "Unrendered Service Benefit",
      isChecked: false,
      isDisabled: false,
    },
  ]);
  ...
  const handleClaimTypeCheckBoxChange = (label: string) => {
    var isChecked: boolean =
      claimTypeCheckBoxList.find((x) => x.label === label)?.isChecked ?? false;
    ...
    setClaimTypeCheckBoxList((current) =>
      current.map((item) =>
        item.label != label ? { ...item, isDisabled: !isChecked } : item,
      ),
    );
  };
  ...
  return (
    <Flex flexDirection="column" gap="4">
      <Grid
        ...
        alignItems="center"
      >
        <FloatingLabelInput label="Incident Date" type="date" />
        <FloatingLabelSelect label="Cause of Incident">
          {reasonList.items.map((item) => (
      ...
      <Flex gap="2" flexDirection="column">
        <Flex flexDirection="column" gap="2">
          <Strong fontSize="14px" color="gray.700">
      ...
      <Flex gap="2" flexDirection="column">
        <Flex flexDirection="column" gap="2">
            ...
            <ClaimsOverlay
              ...
              content={
                ...
                >
                  ...
                  />
                  <FloatingLabelInput label="Date of Birth" type="date" />
                </Grid>
              }
              onConfirm={() => {
          ...
          {payeeList.length === 0 ? (
            ...
            >
              <Body>
                Your payee list is currently empty. Please define a payee.
              </Body>
            </Flex>
          ) : (
            <Grid
