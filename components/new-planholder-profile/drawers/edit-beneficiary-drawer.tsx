...
interface EditBeneficiaryDrawerProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  beneficiary: BeneficiaryProps | null;
  planholderAddress?: string;
  onEdit?: (beneficiary: BeneficiaryProps) => void;
}

export function EditBeneficiaryDrawer({
  open,
  onOpenChange,
  beneficiary,
  planholderAddress,
  onEdit,
}: EditBeneficiaryDrawerProps) {
  ...
  useEffect(() => {
    if (open && beneficiary) {
      ...
      setErrors({});
      setInheritAddress(
        !!planholderAddress && beneficiary.address === planholderAddress,
      );
    }
  }, [open, beneficiary]);
  ...
  const handleSubmit = () => {
    if (!validate() || !beneficiary) return;
    onEdit?.({
      ...beneficiary,
  ...
  const label =
    beneficiary?.beneficiaryClass === "PRINCIPAL"
