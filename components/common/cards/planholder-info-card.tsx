...
export function PlanholderInfoCard({
  planholder,
  insurableBadgeLabel = "INSURABLE",
  weightUnit = "KG",
}: {
  planholder: PlanholderBasicInfo | null;
  insurableBadgeLabel?: string;
  weightUnit?: string;
}) {
  return (
    <Box width={"full"} p={5} my={5} boxShadow={"sm"} borderRadius={"md"}>
      <Flex align={"start"}>
        ...
        <Flex direction={"column"} ml={4} gap={1} w={"full"}>
          ...
          {planholder && (
            <Box>
              <OSPBadge size={"sm"} type={"info"} m={1}>
                {insurableBadgeLabel}
              </OSPBadge>
            </Box>
          )}
      ...
      >
        ...
        <InfoItem
          label="Weight"
          value={
            planholder?.weight ? `${planholder.weight} ${weightUnit}` : "—"
          }
        />
      </Grid>
    </Box>
