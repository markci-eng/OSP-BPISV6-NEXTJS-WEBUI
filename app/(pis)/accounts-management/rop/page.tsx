...
type RopAction = Extract<ActionButtonItem, { type?: "action" }>;

// ROP queues behind the header's "More" ellipsis. None have a destination yet,
// so each reports back instead of navigating.
const OVERFLOW_ACTIONS: RopAction[] = [
  {
    label: "For Recalculation",
    description: "Requests whose ROP amount needs recomputing",
    icon: Calculator,
    onClick: () => toast.info("For Recalculation is not wired up yet."),
  },
  {
    label: "For Release",
    description: "Approved payouts waiting to be released",
    icon: Send,
    onClick: () => toast.info("For Release is not wired up yet."),
  },
  {
    label: "Unclaimed",
    description: "Released payouts the planholder has not collected",
    icon: Inbox,
    onClick: () => toast.info("Unclaimed is not wired up yet."),
  },
  {
    label: "Generate Batch",
    description: "Group requests into a payout batch",
    icon: Layers,
    onClick: () => toast.info("Generate Batch is not wired up yet."),
  },
  {
...
const STATUS_OPTIONS: { label: string; value: RopStatus }[] = [
...
export default function RopPage() {
  ...
  const [requests] = useState<RopRequest[]>(ROP_REQUESTS);
  ...
  // session — coming back from Edit ROP lands on that branch's Pending card
  ...
  const branchOptions = useMemo(
    () => Array.from(new Set(requests.map((r) => r.branchCode))).sort(),
    [requests],
  );
  ...
  const summary = useMemo(() => {
    const scoped = branchSelected
      ? requests.filter((r) => r.branchCode === branch)
      : [];
  ...
  }, [requests, branch, branchSelected]);
  ...
  }, [requests, status, branch, branchSelected]);

  const rowActions = useMemo<RowAction<RopRequest>[]>(
    () => [
      {
        id: "edit",
        label: "Edit Payment",
        icon: Pencil,
        onClick: (row) => router.push(`/accounts-management/rop/${row.id}`),
      },
    ],
    [router],
  );
  ...
  return (
    <Page.Root
      title="ROP"
      description="Return of premium requests."
      headerButton="menu"
    >
      <Page.ToolContent>
        ...
        <Box display={{ base: "block", lg: "none" }}>
          <ActionButtons buttons={OVERFLOW_ACTIONS} title="ROP Requests" />
        </Box>
      </Page.ToolContent>
      ...
      <Page.MainContent>
        ...
        <Flex align="center" gap={3} w="full">
          ...
          {/* Desktop — the first three requests as buttons, then the "More"
              ellipsis holding all five, matching the COFP header */}
          <Flex
        ...
        <DataTable
          ...
          onRowClick={(row) => router.push(`/accounts-management/rop/${row.id}`)}
          features={{
            search: true,
            filtering: true,
            sorting: true,
